...
cd RIchCoin
sudo sh autogen.sh
chmod 777 ~/RIchCoin/share/genbuild.sh
chmod 777 ~/RIchCoin/src/leveldb/build_detect_platform
...
if [ -e ~/RIchCoin/src/qt/RIchCoin-qt ]; then
...
sudo strip ~/RIchCoin/src/RIchCoind
sudo strip ~/RIchCoin/src/qt/RIchCoin-qt
sudo make install
else
echo "Compile fail not RIchCoin-qt present"
fi
...
echo "rpcuser="$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 26 ; echo '') '\n'"rpcpassword="$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 26 ; echo '') '\n'"rpcport=20203" '\n'"port=21203" '\n'"daemon=1" '\n'"listen=1" '\n'"server=1" '\n'"addnode=103.18.40.125:21203" '\n'"addnode=104.131.144.82:21203" '\n'"addnode=138.197.68.130:21203" '\n'"addnode=144.76.167.66:21203" '\n'"addnode=152.186.36.86:21203" '\n'"addnode=159.203.121.202:21203" '\n'"addnode=172.104.157.38:21203" '\n'"addnode=192.99.7.127:21203" '\n'"addnode=219.89.84.46:21203" '\n'"addnode=45.32.129.168:21203" '\n'"addnode=45.55.59.206:21203" '\n'"addnode=46.4.64.68:21203" '\n'"addnode=51.15.46.1:21203" '\n'"addnode=52.9.109.214:21203" '\n'"addnode=66.55.64.183:21203" '\n'"addnode=67.167.207.164:21203" '\n'"addnode=78.46.190.152:21203"> ~/.RIchCoin/RIchCoin.conf

#// Extract http link, download blockchain and install it.

echo -n "Do you wish to download the complete RIchCoin Blockchain (y/n)?"
read answer
if echo "$answer" | grep -iq "^y" ;then
    sudo rm RIchCoin-Blockchain*.zip
    until [ -e RIchCoin*.zip ]
    do
    sleep 1
    echo "wget" $(lynx --dump --listonly https://RIchCoin.de | grep -o "https:*.*zip") > link.sh
    sleep 1
    sh link.sh
    done
    ...
    #checksum
    sudo rm blockchain
    wget https://www.RIchCoin.github.io/checksums/blockchain
    md5sum RIchCoin-Blockchain*.zip > md5
    ...
    unzip -o RIchCoin-Blockchain*.zip -d ~/.RIchCoin
    sudo rm RIchCoin-Blockchain*.zip
else
...
sudo cp ~/RIchCoin/src/qt/res/icons/RIchCoin.png /usr/share/icons/
echo '#!/usr/bin/env xdg-open''\n'"[Desktop Entry]"'\n'"Version=1.0"'\n'"Type=Application"'\n'"Terminal=false"'\n'"Icon[en]=/usr/share/icons/RIchCoin.png"'\n'"Name[en]=RIchCoin"'\n'"Exec=RIchCoin-qt"'\n'"Name=RIchCoin"'\n'"Icon=/usr/share/icons/RIchCoin.png"'\n'"Categories=Network;Internet;" > ~/Desktop/RIchCoin.desktop
sudo chmod +x ~/Desktop/RIchCoin.desktop
sudo cp ~/Desktop/RIchCoin.desktop /usr/share/applications/RIchCoin.desktop
sudo chmod +x /usr/share/applications/RIchCoin.desktop

# Erase all RIchCoin compilation directory , cleaning
...
RIchCoin-qt
cp ~/.RIchCoin/wallet.dat ~/RIchCoinwallet.bak
