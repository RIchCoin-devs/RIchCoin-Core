...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in RIchCoin
    bool fStrictPayToScriptHash = true; // Always active in RIchCoin
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hashPoW > hashTarget && pblock->IsProofOfWork())
        return error("RIchCoinMiner : proof-of-work not meeting target");
...
void RIchCoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        printf("Running RIchCoinMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        {
            if (!NewThread(ThreadRIchCoinMiner, pwallet))
                printf("Error: NewThread(ThreadRIchCoinMiner) failed\n");
            Sleep(10);
        }
    }
}
