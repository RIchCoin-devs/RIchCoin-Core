...
#include "config/RIchCoin-config.h"
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 21203 or testnet: 29102)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 20203 or testnet: 21203)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  RIchCoin is probably already running."), strDataDir.c_str()));
    ...
    if (filesystem::exists(GetDataDir() / "wallet.dat"))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("RIchCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (!fNoListen)
    {
        ...
        if (mapArgs.count("-bind")) {
            ...
            fBound |= Bind(CService(inaddr_any, GetListenPort()), (!fBound) ? (BF_REPORT_ERROR & 0x01) : BF_NONE);
        }
        if (!fBound)
